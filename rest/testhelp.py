...
def lower_keys(x):
    if isinstance(x, list):
      ...
      return UberDict(dict((k.lower(), lower_keys(v)) for k, v in x.items()))
    else:
      return x
...
class TestBase(object):
    ...
    def testFail(self, name, **kwargs):
        print("\t{0}running {1}".format(ConsoleColors.HYELLOW, name).ljust(12), end=' ')
        try:
    ...
    def test(self, name, **kwargs):
        ...
        if not func_name:
            func_name = name
        print("\t{0}running {1}{2}".format(ConsoleColors.HYELLOW, name, ConsoleColors.OFF).ljust(50), end=' ')
        try:
    ...
    def _verifyReturn(self, data, expected):
        ...
        for key in expected:
            ...
            if isinstance(expectedVal, dict):
