...
from rest.models import RestError, requestHasPerms
from rest import helpers, crypto

from account.models import AuthToken, Member, AuthAccount
...
def login_required(func):
    ...
    def inner_func(request=None, *args, **kwargs):
        if not request.user.is_authenticated:
            member = getMemberFromToken(request)
            if not member:
...
class token_bearer(object):
    def __init__(self, bearer=None):
        self.bearer = bearer

    def __call__(self, func):
        def inner_func(request=None, *args, **kwargs):
            updateTokenBearer(request)
            return rest_error_catcher(func, request, *args, **kwargs)
        return inner_func


def updateTokenBearer(request):
    try:
...
def getMemberFromToken(request):
    ...
    if token and len(token) > 4:
        act = AuthAccount.objects.filter(pan=token, kind="authtoken").last()
        if act:
            request.member = act.member
            request.member.touchActivity(last_login=True)
            return request.member
        atoken = AuthToken.objects.filter(token=token).last()
        # removed , token_ip=request.ip from above because ip changed.
        if atoken:
            if atoken.ip and atoken.ip != request.ip:
                return request.member
