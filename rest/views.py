...
try:
	...
	print("recommend installing ujson!")
...
def _returnResults(request, ret, accept_list=None):
	"""
	...
	if not accept_list:
		...
		elif request and request.META.has_key('HTTP_ACCEPT'):
			accept_list = request.META["HTTP_ACCEPT"].split(',')
		elif request and request.META.has_key('HTTP_ACCEPT_ENCODING'):
	...
	if "data" not in accept_list:
		slowdown = getattr(settings, 'DEBUG_SLOWDOWN', 0)
		if slowdown:
			print "SLOWING THINGS DOWN: {0}s".format(slowdown)
			time.sleep(slowdown)
		...
		if hasattr(settings, "DEBUG_REST_END_POINTS") and settings.DEBUG_REST_END_POINTS:
		    for ep in settings.DEBUG_REST_END_POINTS:
		        if request.path.startswith(ep):
		            PersistentLog.log(ret, 20, request, "rest", action="response")
		            break
		else:
			PersistentLog.log(ret, 20, request, "rest", action="response")
	...
	if 'application/json' in accept_list:
		...
		if callback:
			return HttpResponse("%s(%s)" % (callback, toJSON(ret)), content_type="application/json")
		return HttpResponse(toJSON(ret), content_type="application/json")
		...
		return render_to_response("rest_html.html", RequestContext(request,context))
	elif 'text/plain' in accept_list:
		return HttpResponse(pprint.pformat(ret), content_type="text/plain")
	elif 'data' in accept_list:
		return ret
	else:
		return HttpResponse(toJSON(ret), content_type="text/plain")
...
def _filter_recurse(remove, lst):
	...
	for item in lst:
		...
		elif type(item) in (list,tuple) and type(item[0]) in (str, unicode):
			p = item[0]
...
def rest_serialize(data):
	if isinstance(data, datetime.datetime):
		...
		return "{}/{}/{}".format(data.year, data.month, data.day)
...
def updateFeature(func, request, qset, featuresets={}, features=[], **kwargs):
	...
	for f in features:
		...
		if f in featuresets and not f in got_features:
			...
			for p,v in featuresets[f].iteritems():
				p_type = 'A'
				if p[:4] == 'DEL_':
					p_type = 'D'
					p = p[4:]
...
def restGet(request, qset, model=None, fields=None, extra=[], exclude=[], fkey_depth=0, recurse_into=[], filter={}, orig_request=None, orig_objs={}, ignore_noattr=False, require_perms=[], accept_list=None, featuresets={}, features=[], return_httpresponse=True):
	...
	for f in fields:
		...
		fargs = []
		if type(f) in (str, unicode) and "|" in f:
			fargs = f.split("|")
			f = fargs.pop(0)
		...
		if isinstance(data, models.Model):
			...
			elif ObfuscateId in getattr(data.__class__, '__bases__', []):
				ret[fout] = data.obfuscate_id(getattr(qset, f + "_id"))
	...
	for f in recurse_into:
		...
		if f.startswith("generic__") and hasattr(qset, "restGetGenericRelation"):
			...
			if not obj:
				continue
			# print ret
			odata = restGet(request, obj, accept_list=['data'], orig_request=orig_request, **obj.getGraph("generic"))
			odata["model"] = getattr(qset, fout)
			gkey = "{0}_id".format(fout)
			if ret.has_key(gkey):
				del ret[gkey]
		elif hasattr(qset, f) and (hasattr(getattr(qset, f), 'all') or isinstance(getattr(qset, f), list)):
			rset = getattr(qset, f)
			odata = []
			...
			odata = []
		elif hasattr(qset, f) and isinstance(getattr(qset, f), models.Model):
			...
			odata = qset.get(f)
		elif hasattr(qset, f) and hasattr(getattr(qset, f), 'im_func'):
			rset = getattr(qset, f)()
			if rset is not None:
				odata = []
		elif isinstance(qset, dict) and f in qset and hasattr(qset.get(f), 'im_func'):
			rset = qset.get(f)()
			odata = []
		else:
			odata = {}
...
def restListOther(request, qset, size=25, start=0, sort=None, accept_list=None, return_httpresponse=True):
	...
	data_list = []
	for obj in qset:
		if type(obj) in [float, int, long, str, Decimal]:
			...
			print "unhandled type: {}: '{}'".format(type(obj), obj)
...
def restListEx(request, qset, model=None, size=25, start=0, sort=None, fields=None, extra=[], exclude=[], fkey_depth=0, recurse_into=[], filter={}, get_count=True, orig_request=None, orig_objs={}, page=None, ignore_noattr=False, todata=lambda x: x, require_perms=[], accept_list=None, featuresets={}, features=[], return_httpresponse=True):
	...
	if ordering and request:
		try:
			...
			for (o, rev) in ordering:
				...
				if isinstance(page_data[name], dict) and "datetime" in page[name]:
					data = datetime.datetime.fromtimestamp(page[name]["datetime"][0])
					data = data.replace(microsecond=page[name]["datetime"][1])
				elif isinstance(page_data[name], dict) and "date" in page[name]:
	...
	if "key" in extra and "value" in extra and len(extra) == 2 and "*" in exclude:
		...
		if qset:
			# # print fields
			for qr in qset:
				if type(qr) in [float, int, long, str, Decimal]:
					ret['data'].append(qr)
				else:
...
def restPermissionDenied(request, error="permission denied"):
	return restStatus(request, False, error=error, error_code=401)
...
_chunk_badchars = ''.join(list(chr(x) for x in range(0, ord('0')) + range(ord('9')+1, ord('A')) + range(ord('Z')+1, ord('a')) + range(ord('z')+1, 256) ))
...
def generateCSV(qset, fields, name, values_list=False, output=None, stream=False):
	import csv
...
def restCSV(request, qset, fields, name, size=10000):
	if size > 1200 and qset.count() > 1200:
		...
		response['Content-Disposition'] = 'attachment; filename="{0}"'.format(name)
		qset = qset[:size]
		generateCSV(qset, fields, name, output=response)
	return response
...
def showDocs(request):
	...
	return render_to_response("rest_docs.html", {"apis": apis, "graphs":graphs})
